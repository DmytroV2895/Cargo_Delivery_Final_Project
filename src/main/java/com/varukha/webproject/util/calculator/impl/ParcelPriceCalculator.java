...
/**
 * Class DocumentPriceCalculator implements methods for calculating order price by order type - <strong>Parcel</strong>.
 *
 * @author Dmytro Varukha
 * @version 1.0
 */
public class ParcelPriceCalculator implements CalculatorStrategy {
    ...
    public BigDecimal getOrderPrice(String orderType,
        ...
        double parcelVolumeWeight = calculateOrderVolumeWeight(orderLengthToDouble, orderHeightToDouble, orderWidthToDouble, orderWeightToDouble);

        BigDecimal priceByDistance = getPriceByDeliveryDistance(deliveryDistance);
        BigDecimal deliveryPriceByDeliveryType = getPriceByDeliveryType(deliveryType, firstCity, secondCity);

        double priceByDistanceToDouble = Double.parseDouble(String.valueOf(priceByDistance));
        ...
        if (parcelVolumeWeight > 1 && parcelVolumeWeight <= 2) {
            parcelDeliveryPrice = BASE_PRICE_OF_DELIVERY_BY_PARCEL_WEIGHT_1_2_kg +
                    priceByDistanceToDouble + deliveryPriceByDeliveryTypeToDouble;
        }
        if (parcelVolumeWeight > 2 && parcelVolumeWeight <= 10) {
            parcelDeliveryPrice = BASE_PRICE_OF_DELIVERY_BY_PARCEL_WEIGHT_2_10_kg +
                    priceByDistanceToDouble + deliveryPriceByDeliveryTypeToDouble;
        }
        if (parcelVolumeWeight > 10 && parcelVolumeWeight <= 30) {
            parcelDeliveryPrice = BASE_PRICE_OF_DELIVERY_BY_PARCEL_WEIGHT_30_kg +
                    priceByDistanceToDouble + deliveryPriceByDeliveryTypeToDouble;
        }
        BigDecimal parcelDeliveryPriceResult = BigDecimal.valueOf(parcelDeliveryPrice);
        return parcelDeliveryPriceResult.setScale(2, RoundingMode.HALF_UP);
    }
        ...
        switch (deliveryType) {
            case "COURIER" -> {
                if (firstCity.equals(secondCity)) {
                    deliveryPriceByDeliveryType = DELIVERY_PRICE_OF_PARCEL_BY_DELIVERY_TYPE_COURIER_ONE_CITY;
                } else {
                    deliveryPriceByDeliveryType = DELIVERY_PRICE_OF_PARCEL_BY_DELIVERY_TYPE_COURIER_TWO_CITY;
                }
            }
            case "BY_TRUCK" -> {
                if (!firstCity.equals(secondCity)) {
                    deliveryPriceByDeliveryType = DELIVERY_PRICE_OF_PARCEL_BY_DELIVERY_TYPE_BY_TRUCK_ONE_CITY;
                } else {
                    deliveryPriceByDeliveryType = DELIVERY_PRICE_OF_PARCEL_BY_DELIVERY_TYPE_BY_TRUCK_TWO_CITY;
                }
            }
        }
    ...
    public BigDecimal getPriceByDeliveryDistance(double deliveryDistance) {
        ...
        return BigDecimal.valueOf(DELIVERY_PRICE_OF_PARCEL_BY_DISTANCE_BY_1_KM * deliveryDistance);
    }
